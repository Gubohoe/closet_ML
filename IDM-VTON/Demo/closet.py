...
# IDM-VTON 디렉토리를 Python Path에 추가
repo_root = '../../IDM-VTON'
...
print("Directory contents:", os.listdir(repo_root))
...
HUMAN_FOLDER = os.path.join(UPLOAD_FOLDER, 'human')
GARMENT_FOLDER = os.path.join(UPLOAD_FOLDER, 'garment')
...
async def upload_images(data: ImageData):
    ...
    try:
        # Human 이미지 처리
        human_image_bytes = requests.get(base64.b64decode(human_image_data))
        human_filepath = os.path.join(HUMAN_FOLDER, 'human_image.png')
        with open(human_filepath, 'wb') as f:
            f.write(human_image_bytes.content)

        # Garment 이미지 처리
        garment_image_bytes = base64.b64decode(garment_image_data)
        garment_filepath = os.path.join(GARMENT_FOLDER, 'garment_image.png')
        with open(garment_filepath, 'wb') as f:
        ...
        result = process_tryon(
            human_image_path='uploads/human/human_image.png',
            garment_image_path='uploads/garment/garment_image.png',
        ...
        result_filepath = os.path.join('../result.png')
        with open(result_filepath, "rb") as img_file:
            result_base64 = base64.b64encode(img_file.read()).decode('utf-8')

        # Return the result
        return JSONResponse(content={"result_image": result_base64})
